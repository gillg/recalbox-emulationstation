...
void GuiMenu::createInputTextRow(GuiSettings *gui, std::string title, const char *settingsID, bool password) {
    ...
    ed = std::make_shared<TextComponent>(window, ((password &&
            SettingsManager::getInstance()->getString(settingsID) != "")
    ...
    auto updateVal = [ed, settingsID, password](const std::string &newVal) {
        ...
        SettingsManager::getInstance()->setString(settingsID, newVal);
    }; // ok callback (apply new value to ed)
    
    row.makeAcceptInputHandler([this, title, updateVal, settingsID] {
        if (SettingsManager::getInstance()->getBool("UseOSK")) {
            mWindow->pushGui(
                new GuiTextEditPopupKeyboard(mWindow, title, SettingsManager::getInstance()->getString(settingsID),
                                        updateVal, false));
        } else {
            mWindow->pushGui(
                new GuiTextEditPopup(mWindow, title, SettingsManager::getInstance()->getString(settingsID),
                                       updateVal, false));
        }
    });
    gui->addRow(row);
...
GuiMenu::GuiMenu(Window *window) : GuiComponent(window), mMenu(window, _("MAIN MENU").c_str()), mVersion(window) {
    ...
    addEntry(_("GAMES SETTINGS").c_str(), 0x777777FF, true,
             [this] {
                 ...
                 s->addSaveFunc([ratio_choice] {
                     SettingsManager::getInstance()->setString("global.ratio", ratio_choice->getSelected());
                     SettingsManager::getInstance()->save();
                 });
                 // smoothing
                 auto smoothing_enabled = std::make_shared<SwitchComponent>(mWindow);
                 smoothing_enabled->setState(SettingsManager::getInstance()->getString("global.smooth") == "1");
                 s->addWithLabel(_("SMOOTH GAMES"), smoothing_enabled);

                 // rewind
                 auto rewind_enabled = std::make_shared<SwitchComponent>(mWindow);
                 rewind_enabled->setState(SettingsManager::getInstance()->getString("global.rewind") == "1");
                 s->addWithLabel(_("REWIND"), rewind_enabled);

                 // autosave/load
                 auto autosave_enabled = std::make_shared<SwitchComponent>(mWindow);
                 autosave_enabled->setState(SettingsManager::getInstance()->getString("global.autosave") == "1");
                 ...
                 auto shaders_choices = std::make_shared<OptionListComponent<std::string> >(mWindow, _("SHADERS SET"),
                                                                                            false);
                 std::string currentShader = SettingsManager::getInstance()->getString("global.shaderset");
                 ...
                 integerscale_enabled->setState(SettingsManager::getInstance()->getString("global.integerscale") == "1");
                 s->addWithLabel(_("INTEGER SCALE (PIXEL PERFECT)"), integerscale_enabled);
                 s->addSaveFunc([integerscale_enabled] {
                     SettingsManager::getInstance()->setString("global.integerscale", integerscale_enabled->getState() ? "1" : "0");
                     SettingsManager::getInstance()->save();
                 });
                 ...
                 // Retroachievements
                 {
                     ...
                     std::function<void()> openGui = [this] {
                         ...
                         retroachievements_enabled->setState(
                                 SettingsManager::getInstance()->getString("global.retroachievements") == "1");
                         ...
                         retroachievements_hardcore_enabled->setState(
                                 SettingsManager::getInstance()->getString("global.retroachievements.hardcore") == "1");
                         ...
                         retroachievements->addSaveFunc([retroachievements_enabled, retroachievements_hardcore_enabled] {
                             SettingsManager::getInstance()->setString("global.retroachievements",
                                                              retroachievements_enabled->getState() ? "1" : "0");
                             SettingsManager::getInstance()->setString("global.retroachievements.hardcore",
                 ...
                 s->addSaveFunc([smoothing_enabled, rewind_enabled, shaders_choices, autosave_enabled] {
                     SettingsManager::getInstance()->setString("global.smooth", smoothing_enabled->getState() ? "1" : "0");
                     SettingsManager::getInstance()->setString("global.rewind", rewind_enabled->getState() ? "1" : "0");
                     SettingsManager::getInstance()->setString("global.shaderset", shaders_choices->getSelected());
                     SettingsManager::getInstance()->setString("global.autosave", autosave_enabled->getState() ? "1" : "0");
                     SettingsManager::getInstance()->save();
                 });
                 mWindow->pushGui(s);
             }
    ...
    addEntry(_("CONTROLLERS SETTINGS").c_str(), 0x777777FF, true, [this] { this->createConfigInput(); });
      ...
      addEntry(_("UI SETTINGS").c_str(), 0x777777FF, true,
         ...
         auto s = new GuiSettings(mWindow, _("UI SETTINGS").c_str());
                     ...
                     // language choice
                     auto language_choice = std::make_shared<OptionListComponent<std::string> >(mWindow, _("LANGUAGE"),
                                                                                                false);
    ...
    addEntry(_("SOUND SETTINGS").c_str(), 0x777777FF, true,
	       ...
	       auto s = new GuiSettings(mWindow, _("SOUND SETTINGS").c_str());
                 ...
                 sounds_enabled->setState(!(SettingsManager::getInstance()->getString("audio.bgmusic") == "0"));
                 ...
                 std::string currentDevice = SettingsManager::getInstance()->getString("audio.device");
                 ...
                 if (SettingsManager::getInstance()->getString("system.es.menu") != "bartop") {
		   s->addWithLabel(_("OUTPUT DEVICE"), output_list);
                 ...
                 s->addSaveFunc([output_list, currentDevice, sounds_enabled, volume] {
                     ...
                     SettingsManager::getInstance()->setString("audio.volume",
                                                      std::to_string((int) round(volume->getValue())));

                     SettingsManager::getInstance()->setString("audio.bgmusic",
                                                      sounds_enabled->getState() ? "1" : "0");
                     if (!sounds_enabled->getState())
                         AudioManager::getInstance()->stopMusic();
                     if (currentDevice != output_list->getSelected()) {
                         SettingsManager::getInstance()->setString("audio.device", output_list->getSelected());
...
void GuiMenu::popSystemConfigurationGui(SystemData *systemData, std::string previouslySelectedEmulator) const {
    ...
    for (auto it = systemData->getEmulators()->begin(); it != systemData->getEmulators()->end(); it++) {
        ...
        if (previouslySelectedEmulator != "") {
            ...
            found = (SettingsManager::getInstance()->getString(systemData->getName() + ".emulator") == curEmulatorName);
        }
        if (found) {
    ...
    for (auto emulator = systemData->getEmulators()->begin();
        ...
        if (selectedEmulator == emulator->first) {
            for (auto core = emulator->second->begin(); core != emulator->second->end(); core++) {
                bool found = (SettingsManager::getInstance()->getString(systemData->getName() + ".core") == *core);
                selected = selected || found;
    ...
    // smoothing
    auto smoothing_enabled = std::make_shared<SwitchComponent>(mWindow);
    smoothing_enabled->setState(SettingsManager::getInstance()->get(systemData->getName() + ".smooth", SettingsManager::getInstance()->getString("global.smooth")) == "1");
    systemConfiguration->addWithLabel(_("SMOOTH GAMES"), smoothing_enabled);
    // rewind
    auto rewind_enabled = std::make_shared<SwitchComponent>(mWindow);
    rewind_enabled->setState(
            SettingsManager::getInstance()->get(systemData->getName() + ".rewind", SettingsManager::getInstance()->getString("global.rewind")) == "1");
    systemConfiguration->addWithLabel(_("REWIND"), rewind_enabled);
    // autosave
    auto autosave_enabled = std::make_shared<SwitchComponent>(mWindow);
    autosave_enabled->setState(
            SettingsManager::getInstance()->get(systemData->getName() + ".autosave", SettingsManager::getInstance()->getString("global.autosave")) == "1");
    ...
    systemConfiguration->addSaveFunc(
            [systemData, smoothing_enabled, rewind_enabled, ratio_choice, emu_choice, core_choice, autosave_enabled] {
                if(ratio_choice->changed()){
                    SettingsManager::getInstance()->setString(systemData->getName() + ".ratio",
                                                     ratio_choice->getSelected());
                }
                if(rewind_enabled->changed()) {
                    SettingsManager::getInstance()->setString(systemData->getName() + ".rewind",
                                                     rewind_enabled->getState() ? "1" : "0");
                }
                if(smoothing_enabled->changed()){
                    SettingsManager::getInstance()->setString(systemData->getName() + ".smooth",
                                                     smoothing_enabled->getState() ? "1" : "0");
                }
                if(emu_choice->changed()) {
                    SettingsManager::getInstance()->setString(systemData->getName() + ".emulator", emu_choice->getSelected());
                }
                if(core_choice->changed()){
                    SettingsManager::getInstance()->setString(systemData->getName() + ".core", core_choice->getSelected());
                }
                if(autosave_enabled->changed()) {
                    SettingsManager::getInstance()->setString(systemData->getName() + ".autosave",
...
std::shared_ptr<OptionListComponent<std::string>> GuiMenu::createRatioOptionList(Window *window,
    ...
    std::string currentRatio = SettingsManager::getInstance()->getString(configname + ".ratio");
